...
export class StartsComponent implements OnInit {
  ...
  getJoining(teamId,userType){
     ...
     // 1=>sales,2=>Recruitment,3=>Account Management,4=>IT,5=>HR,6=>Legal,7=>Admin,8=>Management,9=>Inside Sales,10=>Marketing 
    // call users joining
      ...
      resp => {
           ...
           if(resp['status_code'] == 200){
               ...
               for(let i=0; i<joinings.length; i++) {
                 ...
                 if(this.usersInterviews.find((itmInner) => itmInner.id === joinings[i].id)){
                     this.merged.push({
                     ...joinings[i], 
